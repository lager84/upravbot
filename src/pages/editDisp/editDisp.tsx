...
const[filteredObjects , setfilteredObjects] = useState([]);
     ...
     let { id } = useParams();
      ...
      const { data:data_gdi, loading:loading_gdi, error:error_gdi} = useQuery(READ_DISP_ITEM, {
       variables: { id },
     });
       ...
       active:data_gdi?.sprDisp[0]?.dispStatus||true,
     ...
     const client_ID = userInfo.userID;
         ...
         const { data:data_go, loading:loading_go, error:error_go} = useQuery(GET_OBJECTS, {
        variables: { client_ID },
      });
     ...
     const [
       ...
       if (data?.usersInfo) {
           ...
           setDispetcher(data?.usersInfo.filter((item:any) => item.role.includes("Диспетчер")));
           setOtvetstvenniy(data?.usersInfo.filter((item:any) => item.role.includes("Ответственный")));
           setIspolnitel(data?.usersInfo.filter((item:any) => item.role.includes("Исполнитель")));
   ...
   useEffect(() => {
      ...
      const  selectDispObjects =data_gdi?.sprDisp[0]?.dispObjects.map((gfoid:any)=>gfoid.gilFindObjectsId);

         const filteredObjects = data_go.gilFindObjects.filter((obj: any) => {
       // Приводим selectDispObjects к массиву с защитой от undefined
       const selectedIds = Array.isArray(selectDispObjects) ? selectDispObjects : [];
   
         const selectfilteredObjects = data_go.gilFindObjects.filter((obj: { id: number; }) => 
    ...
    //    });
       ...
       >
           ...
           Активна
               ...
               <Field name="objects" component={DispObjects} filteredObjects={filteredObjects} selectDispObjects={selectDispObjects} />
               <Field name="dispetcher" component={DispUsers} users={dispetcher} headerText="Диспетчер" selectUsers={data_gdi?.sprDisp[0]?.dispUsers .filter((dispetcher: any) => dispetcher.roleId === "DAA3E2D6-F57F-4906-B9C3-788B37DCA1C5").map((dispetcher:any)=>dispetcher.userId) || []} />
               <Field name="otvetstvenniy" component={DispUsers} users={otvetstvenniy} headerText="Ответственный" selectUsers={data_gdi?.sprDisp[0]?.dispUsers .filter((dispetcher: any) => dispetcher.roleId === "B6FAFCA4-67E0-4CC6-ACE7-F285DEF6A0B6").map((dispetcher:any)=>dispetcher.userId) || []} />
               <Field name="ispolnitel" component={DispUsers} users={ispolnitel} headerText="Исполнитель"  selectUsers={data_gdi?.sprDisp[0]?.dispUsers .filter((dispetcher: any) => dispetcher.roleId === "CD1FEC0D-7828-4C6A-8BE3-25EF9E1A12C3").map((dispetcher:any)=>dispetcher.userId) || []}/>
