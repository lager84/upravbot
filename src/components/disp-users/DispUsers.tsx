...
const DispUsers = ( {headerText, users, selectUsers, field, form }: {headerText:string, selectUsers:string[], users:UsersType[] ,field: any, form: Form }) => {
...
useEffect(() => {
  ...
  setSelectedUsers(users?.filter((user:any) => selectedIds.includes(user.userId)));
},[users])
...
const { name, value } = field;
 ...
 <DataTable value={users} selectionMode={rowClick ? null : 'checkbox'} scrollHeight='400px' selection={selectedUsers} onSelectionChange={(e:any) => {setSelectedUsers(e.value); form.setFieldValue( name, e.value);}} dataKey="userId" tableStyle={{ minWidth: '50rem' }}
